...
class Generator(nn.Module):
    ...
    def forward(self, x, c_org, c_trg, pitch_org, pitch_trg):
        # codes is a LIST of tensors                
        codes = self.encoder(x, c_org)
        # if no c_trg given, then just return the formatted encoder codes
        if c_trg is None:
            """ This part is for when you only need the bottleneck codes for self-similarity"""
        ...
        # concat reformated encoder output with target speaker embedding (which is modified to have a temporal dimension)
        encoder_outputs = torch.cat((code_exp, c_trg.unsqueeze(1).expand(-1,x.size(1),-1)), dim=-1)
        encoder_outputs = torch.cat((encoder_outputs, pitch_org), dim=-1)
